...
test_stage () {
    ...
    echo "STAGE $1"
    echo "===================Valid Programs==================="
    for prog in $(find $root/$stage/valid -type f -name '*.c' | grep --invert multifile); do
        gcc -w $prog
        run_correct_program
    ...
    for dir in `ls -d $root/$stage/valid_multifile/* 2>/dev/null` ; do
        gcc -w $dir/*
    ...
    for prog in `ls $root/$stage/invalid/{,**/}*.c 2>/dev/null`; do

        base="${prog%.*}" #name of executable (filename w/out extension)
        test_name="${base##*invalid/}"

        $ncc $prog >/dev/null 2>&1
